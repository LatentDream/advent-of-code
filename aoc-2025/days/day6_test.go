...
func Test_day6Parse(t *testing.T) {
	tests := []struct {
		name    string
		content string
		want    []Input
...
`,
			want: []Input{
				Input{number: []int{123, 45, 6}, operator: mult},
				Input{number: []int{328, 64, 98}, operator: plus},
				Input{number: []int{51, 387, 215}, operator: mult},
				Input{number: []int{64, 23, 314}, operator: plus},
			},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := day6Parse(tt.content)
